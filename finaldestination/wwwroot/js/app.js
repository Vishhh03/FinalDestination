...
App.pages.home = async function () {
  ...
  try {
    ...
    Utils.$("#featured-hotels").innerHTML = featured
      .map((h) => Components.hotelCard(h))
      .join("");
  } catch (error) {
    Utils.$("#featured-hotels").innerHTML = Components.emptyState(
...
App.pages.hotels = async function () {
  ...
  async function loadHotels(params = {}) {
    try {
      ...
      Utils.$("#hotels-list").innerHTML = Components.emptyState(
        "Unable to load hotels",
        "😔"
      );
    }
    ...
    Utils.$("#hotel-detail").innerHTML = `
            <div class="card">
                <div class="hotel-image" style="height: 400px; border-radius: 16px; overflow: hidden; margin-bottom: 2rem;">
                    <img src="https://picsum.photos/1200/400?random=${
                      hotel.id
      ...
      hotel.name
                ...
                <div class="hotel-location mb-3">
                    📍 ${Utils.escapeHtml(hotel.address)}, ${Utils.escapeHtml(
      hotel.city
    )}
                </div>
                <div class="hotel-rating mb-4">
                    <span class="stars">${Utils.generateStars(
                      hotel.rating || 0
                    )}</span>
                    <span class="rating-text">${(hotel.rating || 0).toFixed(
                      1
                ...
                <div class="grid grid-2 mb-4">
                    <div class="card">
                        <h3>Price</h3>
                        <div class="price-amount">${Utils.formatCurrency(
                          hotel.pricePerNight
                        )}</div>
                ...
                ${
                  Auth.isLoggedIn()
                    ? `
                    <button class="btn btn-primary btn-lg" onclick="App.showBookingForm(${
                      hotel.id
...
App.pages.bookings = async function () {
  ...
  try {
    ...
    Utils.$("#bookings-list").innerHTML = Components.emptyState(
...
App.pages.loyalty = async function () {
  ...
  try {
    ...
    Utils.$("#loyalty-content").innerHTML = `
            <div class="grid grid-2 mb-4">
                <div class="card" style="background: linear-gradient(135deg, var(--gold), var(--gold-light)); color: var(--primary);">
                    ...
                    <div class="price-amount" style="color: var(--primary);">${
                    ...
                    <div class="card mb-2">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                ...
                                <div class="text-secondary">${Utils.formatDate(
                                  t.createdAt
                                )}</div>
                            </div>
    ...
    Utils.$("#loyalty-content").innerHTML = Components.emptyState(
...
App.pages.admin = async function () {
  ...
  app.innerHTML = `
        <section class="section">
            <div class="container">
                ...
                <div id="applications-list">${Components.loadingSkeleton(
                  3
                )}</div>
            </div>
        </section>
  ...
  try {
                ...
                <div class="card mb-3">
                    <div class="card-header">
                        <div>
                            ...
                            <p class="text-secondary">${Utils.escapeHtml(
                              app.userEmail
                            )}</p>
                        </div>
                    ...
                    <div class="card-body">
                        <div class="grid grid-2">
                            <div><strong>Contact:</strong> ${Utils.escapeHtml(
                              app.contactPerson
                            )}</div>
                            <div><strong>Phone:</strong> ${Utils.escapeHtml(
                              app.businessPhone
                            )}</div>
                            <div><strong>License:</strong> ${Utils.escapeHtml(
                              app.businessLicense
                            )}</div>
                            <div><strong>Applied:</strong> ${Utils.formatDate(
                              app.applicationDate
                            )}</div>
                        </div>
                        ${
                          app.additionalInfo
                            ? `<p class="mt-2"><strong>Info:</strong> ${Utils.escapeHtml(
                                app.additionalInfo
                              )}</p>`
                            : ""
                        }
                        ...
                        ${
                          app.adminNotes
                            ? `<p class="mt-2 text-secondary"><strong>Notes:</strong> ${Utils.escapeHtml(
                                app.adminNotes
                              )}</p>`
                            : ""
                        }
    ...
    Utils.$("#applications-list").innerHTML = Components.emptyState(
...
App.processApplication = async function (id, status) {
  const notes = prompt(
    `Enter notes for this ${status.toLowerCase()} decision:`
  );
  ...
  app.innerHTML = `
        <section class="section">
            <div class="container" style="max-width: 700px;">
                ...
                <div id="application-status">${Components.loadingSkeleton(
                  1
                )}</div>
            </div>
        </section>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="grid grid-2 mb-3">
                        <div><strong>Business Address:</strong><br>${Utils.escapeHtml(
                          application.businessAddress
                        )}</div>
                        <div><strong>License:</strong><br>${Utils.escapeHtml(
                          application.businessLicense
                        )}</div>
                        <div><strong>Contact:</strong><br>${Utils.escapeHtml(
                          application.contactPerson
                        )}</div>
                        <div><strong>Phone:</strong><br>${Utils.escapeHtml(
                          application.businessPhone
                        )}</div>
                        <div><strong>Email:</strong><br>${Utils.escapeHtml(
                          application.businessEmail
                        )}</div>
                        <div><strong>Applied:</strong><br>${Utils.formatDate(
                    ...
                    ${
                      application.processedDate
                        ...
                        <p class="text-secondary mt-3">
                            Processed on ${Utils.formatDate(
                              application.processedDate
                            )}
                            ${
                              application.processedByName
                                ? ` by ${Utils.escapeHtml(
                                    application.processedByName
                                  )}`
                                : ""
...
App.showBookingForm = async function (hotelId, hotelName, pricePerNight) {
  // Fetch loyalty account
  let loyaltyAccount = null;
  try {
    loyaltyAccount = await API.loyalty.getAccount();
  } catch (error) {
    ...
    `
        <form id="booking-form">
            ...
            <div class="form-group">
                ...
                <input type="text" id="guestName" class="form-input" value="${
                  Auth.currentUser?.name || ""
                }" required>
            </div>
            <div class="form-group">
                <label class="form-label">Guest Email</label>
                <input type="email" id="guestEmail" class="form-input" value="${
                  Auth.currentUser?.email || ""
                }" required>
            </div>
            <div class="form-group">
                <label class="form-label">Check-in Date</label>
                <input type="date" id="checkInDate" class="form-input" min="${
                  new Date().toISOString().split("T")[0]
                }" required>
            </div>
            <div class="form-group">
                <label class="form-label">Check-out Date</label>
                <input type="date" id="checkOutDate" class="form-input" min="${
                  new Date().toISOString().split("T")[0]
                }" required>
            </div>
            <div class="form-group">
            ...
            ${
              loyaltyAccount
                ? `
                <div class="card mb-3" style="background: var(--bg-secondary); padding: 1rem;">
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        <label class="form-label">Redeem Points (1 point = ₹0.10)</label>
                        <input type="number" id="pointsToRedeem" class="form-input" min="0" max="${
                          loyaltyAccount.pointsBalance
                        }" value="0" placeholder="Enter points to redeem">
                        <small class="text-secondary">Max: ${
                          loyaltyAccount.pointsBalance
                        } points = ₹${(
                    loyaltyAccount.pointsBalance * 0.1
                  ).toFixed(2)} discount</small>
                    </div>
                </div>
            `
                : ""
  ...
  // Calculate price on date change or points change
  const updatePrice = async () => {
    const checkIn = Utils.$("#checkInDate")?.value;
    const checkOut = Utils.$("#checkOutDate")?.value;
    const pointsInput = Utils.$("#pointsToRedeem");
      ...
      preview.innerHTML = `
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    <div style="display: flex; justify-content: space-between;">
                        <span><strong>Base Price:</strong> ${nights} night${
        nights !== 1 ? "s" : ""
      } × ${Utils.formatCurrency(pricePerNight)}</span>
                        <span>${Utils.formatCurrency(baseTotal)}</span>
                    </div>
                    ${
                      discount > 0
                        ? `
                        <div style="display: flex; justify-content: space-between; color: var(--success);">
                            <span><strong>Loyalty Discount:</strong> ${points} points</span>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <span class="price-amount" style="font-size: 1.5rem;">${Utils.formatCurrency(
                          finalTotal
                        )}</span>
                    </div>
                </div>
  ...
  Utils.$("#pointsToRedeem")?.addEventListener("input", updatePrice);
};
...
App.submitBooking = async function (hotelId, pricePerNight) {
  ...
  try {
    ...
    const pointsInput = Utils.$("#pointsToRedeem");
    const pointsToRedeem = pointsInput ? parseInt(pointsInput.value) || 0 : 0;
    ...
    // Add points redemption if specified
    if (pointsToRedeem > 0) {
    ...
    if (pointsToRedeem > 0) {
      Utils.showToast(
        `Booking created! ${pointsToRedeem} points redeemed. Processing payment...`,
        "success"
      );
    } else {
    ...
    // Show payment form with discounted amount
    Utils.$(".modal-overlay").remove();
    App.showPaymentForm(
      booking.id,
      booking.totalAmount,
      booking.loyaltyDiscountAmount
    );
  } catch (error) {
    Utils.hideLoading(btn);
  }
...
// Show Payment Form
App.showPaymentForm = function (bookingId, amount, discount = 0) {
  const modal = Components.modal(
    "Payment",
    `
        <form id="payment-form">
            ${
              discount > 0
                ? `
                <div class="card mb-3" style="background: var(--success); color: white; padding: 1rem;">
                    <strong>🎉 Loyalty Discount Applied: ${Utils.formatCurrency(
                      discount
                    )}</strong>
                </div>
            `
            ...
            <div class="grid grid-2">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="expiryYear" class="form-input" placeholder="25" maxlength="2" required>
                </div>
            </div>
            <div class="form-group">
...
App.pages.manageHotels = async function () {
  ...
  try {
    ...
    if (hotels.length === 0) {
      list.innerHTML =
        '<div class="empty-state">No hotels yet. Create your first hotel!</div>';
      return;
    }
        ...
        (hotel) => `
            <div class="hotel-manager-card">
                <div class="hotel-manager-info">
                    ...
                    <p><strong>Rating:</strong> ${hotel.rating.toFixed(
                      1
                    )} ⭐</p>
                </div>
                <div class="hotel-manager-actions">
                    <button class="btn btn-secondary" onclick="App.showEditHotelModal(${
                      hotel.id
                    })">
                        Edit
                    </button>
...
App.searchBookings = async function () {
  ...
  Utils.$("#all-bookings-list").innerHTML =
      ...
      (booking) => `
        ...
        booking.status
            ...
            <div class="booking-details">
                <p><strong>Guest:</strong> ${booking.guestName} (${
        booking.guestEmail
      })</p>
                ...
                <p><strong>Total:</strong> $${booking.totalAmount.toFixed(
                  2
                )}</p>
                <p><strong>Booked:</strong> ${new Date(
                  booking.createdAt
                ).toLocaleString()}</p>
            </div>
            <div class="booking-actions">
                <button class="btn btn-secondary" onclick="App.viewBookingDetails(${
                  booking.id
                })">
                    View Details
                </button>
    ...
    const modal = Components.modal(
      ...
      `
            <form id="edit-review-form" class="form">
                ...
                <div class="form-group">
                    <label>Comment *</label>
                    <textarea id="editComment" rows="4" required>${
                      review.comment
                    }</textarea>
                </div>
      ...
      booking.status
                        ...
                        <div class="booking-details">
                            <div class="detail-grid">
                                ...
                                <div class="detail-item">
                                    <strong>Total Amount:</strong>
                                    <span>$${booking.totalAmount.toFixed(
                                      2
                                    )}</span>
                                </div>
                                <div class="detail-item">
